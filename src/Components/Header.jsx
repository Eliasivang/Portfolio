...
import logo2 from '../assets/images/eig-logo.png'
...
function Header() {
  ...
  return (
   <header className={fixed ? 'fixed bg-neutral-900 top-0 z-50 flex justify-center w-full h-24 font-bebas shadow-xl' : 'fixed  top-0 z-50 flex justify-center w-full h-24 font-bebas' }>
    <div  className=" xl:w-[1400px] w-full pl-6 pr-4" >
      <motion.div  whileTap = {{scale :1}} transition={{duration:0.5}} className="flex items-center justify-between w-full py-3" >   
            <motion.div whileTap = {{scale :0.9}} className='w-24 sm:w-auto'>
                <img src={logo} alt="logo" className='w-[67px] cursor-pointer'/>
            </motion.div>
        ...
        <IconContext.Provider value={{size:45}}>
          ...
          <BiMenu size={50} onClick={handleClickk} className={active ? 'hidden' : 'p-1  bg-white rounded-full lg:hidden'}/>
            ...
            <ul className='items-center justify-center hidden w-full gap-12 text-xl text-white lg:flex lg:w-auto' >
        
                <motion.li className='text-center cursor-pointer lg:inline-block' whileHover={{scale: 1.1}} whileTap = {{scale :0.9}}>Sobre Mi</motion.li>
            
                    <motion.li className='text-center cursor-pointer lg:inline-block' whileHover={{scale: 1.1}} whileTap = {{scale :0.9}} >Portfolio</motion.li>
